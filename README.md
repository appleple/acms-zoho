...
あわせて、管理画面の「**接続環境設定**」で **接続環境（本番 / サンドボックス / 開発者）とデータセンター（US / EU / IN / … / JP）** を選び、「**設定を保存**」をクリックします（未設定時は 本番 / US）。テストでサンドボックスに接続する場合は「サンドボックス」を選択して保存してください。トークンは環境ごとに別管理のため、環境を変更したら一度認証解除し、選択し直して保存してから再認証してください。
