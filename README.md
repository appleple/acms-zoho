...
あわせて、管理画面の「**接続環境設定**」で **接続環境（本番 / サンドボックス / 開発者）** を選び、「**設定を保存**」をクリックします（未設定時は 本番）。テストでサンドボックスに接続する場合は「サンドボックス」を選択して保存してください。トークンは環境ごとに別管理のため、環境を変更したら一度認証解除し、選択し直して保存してから再認証してください。

なお **データセンター（US / EU / IN / JP / … ）は認証時に Zoho から自動判定** されるため、手動選択は不要です（Zoho の Multi-DC に対応。認証後に判定結果が表示されます）。

環境を保存したら、Client ID / Client Secret を入力して「認証」ボタンをクリックします。クリックすると、認証トークンを入力するプロンプトが表示されるので、API Console で取得した認証トークンを入力して認証を行います。
