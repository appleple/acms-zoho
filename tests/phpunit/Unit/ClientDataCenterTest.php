 ...
 *
 * 何を保証するか:
 *  - location が対応 DC コードならそれを（大文字・空白は正規化して）ZohoDataCenter で返す
...
{
    /**
     * @return array<string, array{0: ?string, 1: ?string, 2: ?ZohoDataCenter}>
     */
    public static function detectCases(): array
    {
        return [
            // location コードがそのまま対応 DC になる
            'location us' => ['us', 'https://accounts.zoho.com', ZohoDataCenter::US],
            'location jp' => ['jp', 'https://accounts.zoho.jp', ZohoDataCenter::JP],
            'location eu' => ['eu', 'https://accounts.zoho.eu', ZohoDataCenter::EU],
            'location ca' => ['ca', 'https://accounts.zohocloud.ca', ZohoDataCenter::CA],
            // 大文字・前後空白は正規化される
            'location 大文字/空白' => ['  JP ', null, ZohoDataCenter::JP],
            // location が未知でも accounts-server のホストで救済する
            'accounts-server で救済(jp)' => ['', 'https://accounts.zoho.jp/', ZohoDataCenter::JP],
            'accounts-server で救済(au)' => ['unknown', 'https://accounts.zoho.com.au', ZohoDataCenter::AU],
            'accounts-server で救済(cn)' => [null, 'https://accounts.zoho.com.cn', ZohoDataCenter::CN],
    ...
    public function itDetectsDataCenter(?string $location, ?string $accountsServer, ?ZohoDataCenter $expected): void
    {
